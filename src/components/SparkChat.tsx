...
export function SparkChat() {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto flex flex-col h-[82vh] liquid-glass rounded-3xl border border-white/15 overflow-hidden shadow-2xl relative">
      
      {/* Top Header */}
      <div className="px-6 py-4 border-b border-white/10 bg-slate-900/60 backdrop-blur-xl flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="relative">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-violet-600 to-indigo-500 flex items-center justify-center p-[1px]">
          ...
          <div>
            <h2 className="font-display font-bold text-lg text-white flex items-center gap-2">
              Spark AI
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-violet-500/20 text-violet-300 border border-violet-400/20 font-sans font-medium">
                Groq Llama 3.3
              </span>
            </h2>
            <p className="text-xs text-slate-400">Empathic Companion • Zero Logging</p>
          </div>
        </div>

        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-1.5 text-xs text-slate-300 bg-white/5 border border-white/10 px-3 py-1.5 rounded-full">
      ...
      {/* Safety Alert Banner */}
      <div className="px-6 py-2.5 bg-violet-950/40 border-b border-violet-500/20 text-[11px] text-violet-200 flex items-center justify-between gap-2">
        ...
        <Link href="/crisis" className="text-rose-400 hover:text-rose-300 font-semibold underline shrink-0 flex items-center gap-1">
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
        {messages.map((msg) => (
          ...
          >
            ...
            <div className={`w-8 h-8 rounded-full shrink-0 flex items-center justify-center text-xs font-bold ${
              ...
              {msg.role === "user" ? "You" : <Sparkles className="w-4 h-4 text-violet-300" />}
            </div>
        ...
        {loading && (
          ...
          >
            <div className="w-8 h-8 rounded-full bg-slate-800 border border-violet-500/30 flex items-center justify-center">
            ...
            <div className="liquid-glass-card px-4 py-3 rounded-2xl rounded-tl-none flex items-center gap-2">
              ...
              <span className="w-2 h-2 rounded-full bg-violet-400 animate-pulse [animation-delay:0.2s]" />
              <span className="w-2 h-2 rounded-full bg-violet-400 animate-pulse [animation-delay:0.4s]" />
              <span className="text-xs text-slate-400 ml-1">Spark is thinking...</span>
            </div>
          </motion.div>
        )}
        ...
        {isLimited && (
          <motion.div
            ...
            className="p-5 rounded-2xl bg-amber-950/40 border border-amber-500/30 text-amber-200 text-sm flex flex-col gap-3 my-2"
          >
            ...
            <div className="flex gap-3 mt-1">
              <Link href="/calm" className="px-4 py-2 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 border border-amber-500/40 text-xs font-semibold text-amber-100 flex items-center gap-1.5">
                <Wind className="w-3.5 h-3.5" />
                <span>Try Calm Tools</span>
              </Link>
              <Link href="/dashboard" className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-xs font-semibold text-white">
                Log Your Mood
              </Link>
            </div>
          </motion.div>
      ...
      {/* Quick Action Chips */}
      {!isLimited && (
        <div className="px-6 py-2 bg-slate-950/40 border-t border-white/5 flex items-center gap-2 overflow-x-auto no-scrollbar">
          {QUICK_CHIPS.map((chip, idx) => (
            <button
              key={idx}
              onClick={() => handleSendMessage(chip.text)}
              disabled={loading}
              className="px-3.5 py-1.5 rounded-full text-xs font-medium bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 hover:text-white shrink-0 transition-all hover:scale-105"
            >
              {chip.label}
            </button>
          ))}
